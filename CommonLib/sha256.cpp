...
void SHA256::update(const uint8_t *message, size_t len)
{
    size_t tmp_len = SHA224_256_BLOCK_SIZE - m_len;
    ...
    size_t block_nb = new_len / SHA224_256_BLOCK_SIZE;
...
{
    uint32_t block_nb = (1 + ((SHA224_256_BLOCK_SIZE - 9) < (m_len % SHA224_256_BLOCK_SIZE)));
...
std::string sha256(const std::string& input)
{
    ...
    for (uint32_t i = 0; i < SHA256::DIGEST_SIZE; i++)
	    sstream << std::hex << std::setw(2) << std::setfill('0') << static_cast<uint32_t>(digest[i]);

    return sstream.str();
}

bool sha256(const std::string& input, char caBuf[], size_t nBufLen)
{
    std::string strSha256 = sha256(input);
    if (nBufLen >= strSha256.size())
    {
        std::copy(strSha256.begin(), strSha256.end(), caBuf);
        return true;
    }
